...
# 留空时由 SignPipeline 首次运行时生成强随机密码并保存到 0600 文件。
KEYSTORE_PASSWORD = os.environ.get("HAPSIGN_KEYSTORE_PASSWORD", "")

# ── hap 签名 ──────────────────────────────────────────────────
HAP_SIGN_ALG = "SHA256withECDSA"
HAP_COMPATIBLE_VERSION = "20"  # HarmonyHello 的 minAPIVersion
...
DEVECO_JBR = os.environ.get("HAPSIGN_JAVA", shutil.which("java") or "/usr/bin/java")
KEYTOOL_PATH = os.environ.get(
    "HAPSIGN_KEYTOOL", shutil.which("keytool") or "/usr/bin/keytool"
)
