...
def _pipeline(tmp_path, monkeypatch) -> SignPipeline:
    ...
    monkeypatch.setattr("hapsign.pipeline.STATE_DIR", str(tmp_path / "state"))
    monkeypatch.setattr(
        "hapsign.pipeline.SIGNING_FILES_DIR", str(tmp_path / "state" / "signing_files")
    )
    monkeypatch.setattr(
        "hapsign.pipeline.CREDENTIALS_DIR", str(tmp_path / "state" / "credentials")
    )
    return SignPipeline(
        hap_path="app.hap",
...
def test_expired_token_cache_is_ignored(tmp_path, monkeypatch) -> None:
    pipeline = _pipeline(tmp_path, monkeypatch)
    cache_path = pipeline._token_cache_path
...
def test_projects_share_arch_signing_credentials(tmp_path, monkeypatch) -> None:
    first = _pipeline(tmp_path, monkeypatch)
    second = SignPipeline(
        hap_path="second.hap",
        bundle_name="com.example.second",
        work_dir=str(tmp_path / "second-signing"),
    )

    assert first._password_path == second._password_path
    assert first.keystore_password == second.keystore_password
