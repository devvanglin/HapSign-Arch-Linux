...
1. **HarmonyOS Command Line Tools** 已安装（需要 Java、keytool、hap-sign-tool.jar、hdc）
...
本机 Arch 适配版已经安装了快捷命令：

```bash
hapsign-arch --hap /path/to/app-unsigned.hap
```
...
会用当前系统默认浏览器打开华为官方登录页。手动输入账号密码登录，如果有验证码或二次验证也手动处理。登录成功后无需关闭浏览器，工具会自动完成签名和安装。
...
- **Token 缓存**：只在本次流程中临时使用，结束时自动删除
- **签名文件缓存**：`~/.local/state/harmonyos/hapsign/signing_files/{bundle_name}/metadata.json`
...
- 此本地适配版本支持 Arch Linux；仓库原带的 bat 脚本仍仅适用于 Windows
